...
void DecodeEthTypes(Packet *p, const uint8_t *pkt, uint16_t ethtype, uint32_t cap_len, uint8_t linklen)
{
    ...
    switch(ethtype)
    {
        ...
        case ETHERNET_TYPE_GOOSE:
        	p->data = pkt + linklen;
        	p->dsize = cap_len - linklen;
        	p->proto_bits |= PROTO_BIT__GOOSE;
        	PushLayer(PROTO_ETH, p, pkt, sizeof(*p->eh));
        	return;

        case ETHERNET_TYPE_SV:
        	p->data = pkt + linklen;
        	p->dsize = cap_len - linklen;
        	p->proto_bits |= PROTO_BIT__SV;
        	PushLayer(PROTO_ETH, p, pkt, sizeof(*p->eh));
        	return;

        case ETHERNET_TYPE_GSE_MGMT :
        ...
        if(p->ehllc->dsap == ETH_DSAP_IP && p->ehllc->ssap == ETH_SSAP_IP)
        {
            ...
            switch(ntohs(p->ehllcother->proto_id))
            {
                ...
                case ETHERNET_TYPE_QINQ_NS2:
                    ...
                    return;
                case ETHERNET_TYPE_GOOSE:
                	p->data = pkt + LEN_VLAN_LLC_OTHER;
                	        	p->dsize =  len - LEN_VLAN_LLC_OTHER;
                	        	p->proto_bits |= PROTO_BIT__GOOSE;
    ...
    else
    {
        ...
        switch(ntohs(vh->vth_proto))
        {
            ...
            case ETHERNET_TYPE_LOOP:
                ...
                return;
            case ETHERNET_TYPE_GOOSE:
                           	p->data = pkt + sizeof(VlanTagHdr);
                           	        	p->dsize =  len - sizeof(VlanTagHdr);
                           	        	p->proto_bits |= PROTO_BIT__GOOSE;
                           	        	return;
            case ETHERNET_TYPE_SV:
                                            	p->data = pkt + sizeof(VlanTagHdr);
