 ...
 *      Author: rtds-cybersec
 */
//#include "dnp3_chk_crc.h"
//#include "sf_types.h"
//#include "sf_snort_packet.h"
//#include "spp_dnp3.h"
void DNP3StoreCRC(unsigned char *buf, uint16_t buflen);
...
int DNP3PopulateCRC( const int8_t *pdu_start,
                              uint16_t pdu_length)
{
    char *cursor;
    uint16_t bytes_left;
    ...
    /* Check Header CRC */
   DNP3StoreCRC((unsigned char*)pdu_start, sizeof(dnp3_link_header_t)+2);
    ...
    while ( (bytes_left > (DNP3_CHUNK_SIZE + DNP3_CRC_SIZE))  )
    {
            ...
            DNP3StoreCRC((unsigned char*)cursor, (DNP3_CHUNK_SIZE+DNP3_CRC_SIZE));
    ...
    {
       DNP3StoreCRC((unsigned char*)cursor, bytes_left);
...
void DNP3StoreCRC(unsigned char *buf, uint16_t buflen)
{
    ...
    temp_crc = (unsigned char)crc ;
    memcpy(buf[idx],&temp_crc,1);
    idx++;
    temp_crc = (unsigned char)(crc >> 8);
    memcpy(buf[idx],&temp_crc,1);

}

void computeCRC(unsigned char data, uint16_t *crcAccum)
{
    *crcAccum =
