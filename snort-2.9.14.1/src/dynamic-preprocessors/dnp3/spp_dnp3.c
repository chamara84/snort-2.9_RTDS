        ...
        else if (strcmp(token, "change") == 0)
		{
        	...
        	token = strtok_r(NULL, " ,", &saveptr);
        	 ...
        	 if (token == NULL)
        	             {
        	                 DynamicPreprocessorFatalMessage("%s(%d): Missing argument for "
        	                     "DNP3 preprocessor 'change' option.\n",
        	                     *_dpd.config_file, *_dpd.config_line);
        	             }
        	 else{
        		 switch(count){
        		 	 	 	 	 case(0): (config->values_to_alter[index]).func_code = strtol(token,NULL,10);
        		         						 	 break;

        		 	 	 	 	 case(1): (config->values_to_alter[index]).obj_group = strtol(token,NULL,10);
        						 	 break;
        						 case(2): (config->values_to_alter[index]).obj_var = strtol(token,NULL,10);
        						 	 break;
        	...
        	config->values_to_alter[index].operation = 1;
        	index++;
        ...
        else if (strcmp(token, "percentage") == 0)
		{
        	...
        	token = strtok_r(NULL, " ,", &saveptr);
        	 ...
        	 if (token == NULL)
        		 ...
        		 switch(count){
        						 case(0): (config->values_to_alter[index]).obj_group = strtol(token,NULL,10);
        						 	 break;
        						 case(1): (config->values_to_alter[index]).obj_var = strtol(token,NULL,10);
        						 	 break;
        						 case(2): (config->values_to_alter[index]).identifier = strtol(token,NULL,10);
        						 	 break;
        						 case(3):
        								 if((config->values_to_alter[index]).obj_group==1 || (config->values_to_alter[index]).obj_group==10)
        								 {
        									 (config->values_to_alter[index]).integer_value =strtol(token,NULL,10);
        	...
        	config->values_to_alter[index].operation = 2;
        	index++;
        	config->numAlteredVal = index;
		}
        else
...
packetp->flags |= FLAG_ALLOW_MULTIPLE_DETECT;
    ...
    if (packetp->flags & FLAG_FROM_CLIENT)
        sessp->direction = DNP3_CLIENT; //mean a request packet
    else
        sessp->direction = DNP3_SERVER; //mean a response packet
...
//    	}
        ...
        DNP3FullReassembly(dnp3_eval_config, sessp, packetp,(uint8_t *)packetp->payload, packetp->payload_size);
